...
const AddBook: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try{
      const res = await api.post("/admin/addBook", {
  ...
  return (
    <div className=" bg-slate-50 mt-[5rem]">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border p-[2px]"
              required
            />
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {genreOptions.map((g) => (
                <label
                  key={g}
                  className="flex flex-wrap items-center gap-2 p-[5px] text-sm cursor-pointer rounded-3xl hover:bg-slate-200"
                >
                  <input
                    type="checkbox"
          ...
          {error && (
            <div className="px-[1rem] py-[.5rem] text-sm text-red-700 bg-red-100 rounded-lg">
              {error}
            </div>
          )}
          {msg && (
          ...
          <div className="flex justify-center pt-[1.4rem]">
            <ButtonField id="submit" data="Add Book" type='submit'
                        className='w-[80%]
                              p-[7px]
                              rounded-[10px]
                              font-semibold
